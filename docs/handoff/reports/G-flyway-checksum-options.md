...
**한 줄 결론:** 다른 작업자가 현재 작업 트리의 V2·V4·V5를 `4e251a0` 이전 내용으로 원복했다. 코드 비교상 기존 체크섬 불일치 원인은 제거됐지만 기존 DB의 `flyway validate` 결과는 확인하지 않았으므로 운영 판단과 적용 확인은 남아 있다.

## 현재 상태(2026-09-24)

- `git diff 4e251a0^ -- V2/V4/V5`에 내용 차이가 없다. 아래 비교표는 원복 전 결정을 위해 작성한 이력이며, 현재 작업 트리의 기본 방안은 원복 + 새 V18 데모 시드 정리다.
- 현재 V18의 prod 프로필 단독 테스트와 전체 백엔드 319건은 새 PostgreSQL에서 통과했다. 기존 개발·운영 DB의 적용 이력 검증, 데이터 영향 검토 및 V18 실행은 하지 않았다.
- 기존 DB에 원본 V2·V4·V5가 적용됐었다면 `repair` 없이 체크섬이 일치할 것으로 예상된다. 실제 적용 버전과 체크섬이 다를 가능성은 있으므로 배포 전에 읽기 전용 `flyway validate`로 확인해야 한다.
