...
- V16·V17 SQL, 관련 repository 메서드, 기본 엔티티 필드의 대응을 읽기 전용으로 점검했다. 모든 테이블의 컬럼별 length/precision/scale을 DB 카탈로그와 자동 비교하는 도구는 실행하지 않았다. 아래 표는 검증한 범위이며 포괄적 무불일치를 뜻하지 않는다.
...
`String` ↔ `character(1)` Y/N 필드와 숫자 precision/scale은 `ddl-auto=validate`만으로 값의 길이·반올림 정책까지 증명되지 않는다. 이 항목은 운영 카탈로그 조회와 경계값 삽입 시험이 남았다. SQL이나 엔티티는 수정하지 않았다.
